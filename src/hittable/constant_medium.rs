...
impl Hittable for ConstantMedium {
    fn hit(&self, ray: &Ray, t_min: f64, t_max: f64, rng: &mut SmallRng) -> Option<HitRecord> {
        let mut rec1 = match self.boundary.hit(ray, -std::f64::INFINITY, std::f64::INFINITY, rng) {
            Some(hit) => { hit },
            None => { return None; }
        };
        let mut rec2 = match self.boundary.hit(ray, rec1.t+0.0001, std::f64::INFINITY, rng) {
            Some(hit) => { hit },
            None => { return None; }
        };
